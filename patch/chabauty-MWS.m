...
/***********************************************************************
 ...
 October 2012 (sent to Magma in June 2013), Michael Stoll:
  ...
  July 2018, bug fix by Michael Stoll:
    -- when trying to find a rational killing differential,
       entry[5] should only be used (otherwise may not exist)
       when entry[4] is true.
    -- fixed a typo in a verbose message

  December 2019, Michael Stoll:
    -- new version of Saturation, moved to new file saturation.m

  May 2021, Michael Stoll:
    -- prompted by an example for which Chabauty seemingly did not finish,
       moved check for reaching GiveUpBound in Chabauty() to after the
       checking for points and before epsilon is changed.
       (In the previous version, the condition was essentially never
       triggered.) Added increase of PrimeBound and GiveUpBound as
       the default behaviour in this case (unless it can be detected
       that there is a rational killing differential vanishing
       in a rational point).
    -- Added optional flag SanityCheck to Chabauty(). If set to false,
       it prevents the check that all small points have been found.
       This is useful when one wants to find points mapping into
  ...
  for p in Type(bound) eq RngIntElt select [lb..bound by 2] else bound do
    if IsPrime(p) and p notin bp then
      ...
      if Max(PrimeDivisors(oG)) lt SmoothBound then
        ...
        vprintf Chabauty, 3: "     after restriction: group = %o, #C(F_p) = %o\n",
                             Invariants(Gsub), #imC;
...
// The following version of saturation at a prime p
// (using IsDivisibleBy directly rather than Saturation)
...
intrinsic Chabauty(ptJ::JacHypPt : ptC := 0,
                   ...
                   CheckBound := 300, GiveUpBound := 1000, Saturate := true, SanityCheck := true)
...
{For a curve C of genus 2 over Q, this returns the full set of rational points,
 ...
 then the result may be incorrect in this case.}
  ...
  eps := Epsilon; // epsilon value will be changed if necessary
  while true do
    ...
    while est ge eps or IsEmpty(Bs) do
      // information not yet sufficient or no separating B found so far
      if b ge CheckBound and not checked and est lt eps then
      ...
      vprintf Chabauty, 1: "   check value is %o %o Epsilon.\n", est, est ge eps select ">=" else "<";
    end while;
    ...
    run, flag := dryRun1(GI, PrimeBound, 5*eps, #T, Bs);
    if not flag then
...
//             require false:
      ...
      // otherwise, try again with larger PrimeBound
      PrimeBound *:= 2;
      GiveUpBound *:= 2;
      vprintf Chabauty, 1: "\n Some classes unresolved when GiveUpBound is reached\n";
      vprintf Chabauty, 1: "  ==> increase PrimeBound to %o, GiveUpBound to %o and start over.\n\n",
                           PrimeBound, GiveUpBound;
      oldbound := 2; // lower bound for reduction primes
      GI := [];      // GroupInfo structure
      b := 10;       // upper bound for reduction primes (will be doubled)
      est := 1.0;    // will be used for value estimating quality of information obtained
      ...
      // GiveUpBound not yet reached
      vprintf Chabauty, 1:
              not Saturate select "\n Some classes unresolved ==> reduce Epsilon and start over.\n\n"
              else "\n Some classes unresolved ==> reduce Epsilon, improve saturation, and start over.\n\n";
      eps := 0.5*est;
    end if;
    if Saturate then
